...
export class TaskUsersService {
  ...
  async postUserDataOnReg(user: UserDet) {
    return this.http.post('http://mytasknowbackend.azurewebsites.net:8080/api/v1/users',user);
  }

  async postUserDataOnPersonDataProfileEdit(user: UserDet) {
    // console.log("user.getUserId()" + user.getUserId());
    return this.http.put('http://mytasknowbackend.azurewebsites.net:8080/api/v1/users/'+user.getUserId(),user);
  }

  async getUserDataByID(userId: string) {
    this.http.get<UserDet>('http://mytasknowbackend.azurewebsites.net:8080/api/v1/users/'+userId).subscribe(data => {
        this.users = data;
